...
*函数名称：MazeMan()
*函数功能：构造函数，初始化数据成员
*函数参数：man 表示游戏角色的字符       manface 游戏角色的朝向
*修改时间：2016.7.18
**************************/
...
*函数名称：setPosition()
*函数功能：设置游戏角色位置
*函数参数：x 角色所在位置的横坐标       y 角色所在位置的纵坐标
*修改时间：2016.7.18
********************************/
void MazeMan::setPosition(int x, int y)
{
	unsigned long numWritten;   //获得命令行窗口的窗口句柄
	HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);
	m_COORDManCurrentPosition.X = x;
	m_COORDManCurrentPosition.Y = y;   //在指定坐标位置填充指定的字符,这里我们在游戏角色的当前位置填充代表游戏角色的字符，使游戏角色采出现
...
{
	unsigned long numWritten;   //获取命令行窗口的窗口句柄   
	HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);   //在指定坐标位置填充指定的字符,这里我们在游戏角色的当前位置填充空格，使游戏角色消失
	FillConsoleOutputCharacter(handle, m_pMap->m_cRoad, 1, m_COORDManCurrentPosition, &numWritten);
	switch (direct)
	{
	case U:     m_COORDManCurrentPosition.Y -= 1;
		break;
	case D:     m_COORDManCurrentPosition.Y += 1;
		break;
	case L:     m_COORDManCurrentPosition.X -= 1;
		break;
	case R:     m_COORDManCurrentPosition.X += 1;
		break;
	default:
		break;
	}   //游戏角色坐标更新，在新的坐标  
	FillConsoleOutputCharacter(handle, m_cMan, 1, m_COORDManCurrentPosition, &numWritten);   //移动完成后跟新游戏角色的朝向  
	m_cManFace = direct;
}
/************************************************
*函数名称：walkUp()
...
void MazeMan::start()
{
	while (true)
	{
		...
		switch (m_cManFace)
		{
		case U:       //游戏角色朝上时，先向右，再向上，再向左，最后向下（右手原则）
			walkRight() || walkUp() || walkLeft() || walkDown();
			break;
		case D:       walkLeft() || walkDown() || walkRight() || walkUp();
			break;
		case L:       walkUp() || walkLeft() || walkDown() || walkRight();
			break;
		case R:       walkDown() || walkRight() || walkUp() || walkLeft();
			break;
		default:
			break;
		}
		...
		if (m_COORDManCurrentPosition.X == m_pMap->m_COORDExitPostion.X && m_COORDManCurrentPosition.Y == m_pMap->m_COORDExitPostion.Y)
			break;
		std::cout << "已经走了" << m_iSteps - 1 << "步";     Sleep(500);
	}
}
